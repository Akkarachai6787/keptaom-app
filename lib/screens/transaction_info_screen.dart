...
class _TransactionInfoScreenState extends State<TransactionInfoScreen> {
  ...
  Future<void> fetchTypeAndWalletNames() async {
    try {
      ...
      if (typeName == 'budget') {
        budgetList = await budgetServices.fetchBudgets(widget.userId);
      }
      ...
      if (widget.transaction.isTransfer) {
        final toWallet = await walletServices.fetchWalletById(
          widget.transaction.toWalletId!,
        );
        toWalletName = toWallet?.label;
      }
    } catch (e) {
                      ...
                      if (isTransfer == true) {
                        ...
                        if (typeName == 'budget') {
                          await budgetServices.updateBudgetBalance(
                            id: budgetList.first.id,
                            amount: amount,
                            isIncome: true,
                            currentAmount: budgetList.first.amount
                          );
                        }
